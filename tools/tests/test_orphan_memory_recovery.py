...
`activate_orphaned_memories()` must enforce the *configured*
LLMChatter.Memory.MaxPerBotPlayer when it trims a pair back to the
cap. A hardcoded cap here would silently delete memories above the
configured limit on the next bridge start. These tests pin that.
