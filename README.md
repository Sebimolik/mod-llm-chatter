...
* **Gameplay Savvy**: Your companions can answer questions about the game itself, not just chat about it. Ask where to learn a profession, when you'll be able to fly, or what level a dungeon is for, and they'll answer accurately in character — drawing from a curated Wrath-era knowledge base of professions, classes, mounts, talents, dungeons, zones and mechanics, matched to your question in English, Russian, French, German or Spanish. Inspired by the RAG feature in [mod-ollama-chat](https://github.com/DustinHendrickson/mod-ollama-chat), reimplemented as a lightweight keyword lookup that needs no extra LLM call.
...
### 2026-08-22 - Companions Answer Gameplay Questions

Bots can now field questions about the game itself. Ask how to become an alchemist, when you'll be able to fly, or where a dungeon is, and they'll answer accurately and in character from a curated Wrath knowledge base covering professions, classes, mounts, talents, dungeons, zones and mechanics — in English, Russian, French, German and Spanish. The feature was inspired by the RAG knowledge retrieval in [mod-ollama-chat](https://github.com/DustinHendrickson/mod-ollama-chat), reimplemented as a lightweight keyword lookup that needs no extra LLM call and no new schema.

### 2026-08-22 - Memory Hardening & Stability Pass

The memory system got a thorough reliability pass. Bots no longer risk folding away a real memory of you by accident when tidying up, and condensed summaries now fade naturally over time instead of lingering forever. Relationships stay consistent even after a big tidy-up, and a rare edge case where a not-yet-saved memory could shove out a genuine one is gone. Behind the scenes, config defaults now match the shipped template, stale gear-and-mount reaction bookkeeping is cleaned up so companions stay responsive through long sessions, and the module ships a new regression test that keeps the memory "importance over time" logic locked in sync between the server and the bridge. Nothing new to learn — bots just remember you more faithfully and stay lively longer.
